...
//	easyui的dialog提供的方法，此处是先执行open方法，然后对本对话框设置标题
	$('#dialog').dialog('open').dialog('setTitle', '创建${functionName}');
//	初始化表单，将表单清空
	$('#form').form('clear');
...
//	判断${functionName}选中的是单行还是多行，编辑的时候只能选中单行，多行的时候只能修改第一个选中的记录
	if(rows.length!=1){
...
//			easyui的打开dialog的方法，并且设置新标题
			$('#dialog').dialog('open').dialog('setTitle', '编辑${functionName}');
//			初始化表单，清空所有组件的value值。
			$('#form').form('clear');
...
$(function(){
	...
	$('#datagrid').datagrid({
		...
		url :  contextPath+'/listpaged${classname}s.do',//载入数据的后台方法，后台返回的数据必须放在rows变量中。datagrid才能自动识别它。
		method : "POST",//请求方式
		pageSize : 20,
