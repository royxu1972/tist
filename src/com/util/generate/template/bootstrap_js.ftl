...
function create(){
	...
	//设置对话框标题
	$("#dialog_title").text("新增${functionName}");
	//打开对话框
	$('#dialog').modal('show');
	//easyui清空表单的操作
	...
	//保存方法,在${ClassName}Controller.java中
...
function edit(){
	...
	//清空验证状态(于public/js/public.js中)
	removeValidate("#data_form");
	$("#dialog_title").text("编辑${functionName}");
	...
	//修改方法,在${ClassName}Controller.java中
...
function remove(){
	...
	if(rows.length>0){
		$.messager.confirm('Confirm', '确定要删除?', function(r) {
			if (r) {
				//删除方法,在${ClassName}Controller.java中
				url = contextPath + "/delete${classname}s.do";
				method = "POST";
				data_form._method.value="DELETE";
				for ( var i = 0; i < rows.length; i++) {
					$("#ids").append(
							"<input type=\"hidden\" name=\"ids\" value=\"" + rows[i].${classname}_id
									+ "\">");
					}
				$('#data_form').form('load', rows[0]);
				$('#submit').click();
...
$(function(){
	...
	$('#data_table').bootstrapTable({
	    method: 'GET',
	    url: contextPath+'/listpaged${classname}s.do',//分页查询,在${ClassName}Controller.java中
	    cache: false,
	    height: 450,
	    striped: true,
	    pagination: true,
...
//			获取当前按钮的iconCls的值，根据值来判断用户点击了什么按钮
	...
	$('#submit').click(function() {
		$.ajax({
			...
			url : url,
			beforeSend:function(){
				validateForm();//在${classname}_validate.js中
