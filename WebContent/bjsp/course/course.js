...
function create(){
	...
	//保存方法,在CourseController.java中
	url = contextPath + "/savecourse.do";
	method = "POST";
...
//				console.info("current_name="+current_name+"\n"+"current_path="+new_name);
				var file_link = "<div id='cur_file_"+i+"'>" +
									"<a target='_blank' href='"+contextPath+"/download.do?file_name="+new_name+"&old_name="+encodeURI(encodeURI(current_name))+"'>"+current_name+"</a>" +
									"&nbsp;&nbsp;" +
	...
	//修改方法,在CourseController.java中
	url = contextPath+"/updatecourse.do";
    method = "POST";
    data_form._method.value="POST";
}
...
function remove(){
	...
	if(rows.length>0){
		$.messager.confirm('Confirm', '确定要删除?', function(r) {
			if (r) {
				//删除方法,在CourseController.java中
				url = contextPath + "/deletecourses.do";
				method = "POST";
				data_form._method.value="DELETE";
				for ( var i = 0; i < rows.length; i++) {
					$("#ids").append(
							"<input type=\"hidden\" name=\"ids\" value=\"" + rows[i].course_id
									+ "\">");
					}
				$('#data_form').form('load', rows[0]);
				$('#submit').click();
...
//	alert(file_id+"\t"+notice_id);
	if(valueIsNotEmpty(file_id)&&valueIsNotEmpty(related_id)){
		$.ajax({
			type : "POST",
			data : {attachment_id:file_id,related_id:related_id,file_path:path,table_of_attachment:"roy_course"},
...
//			$('#summernote').code($(this).val());
	...
	$('#data_table').bootstrapTable({
	    method: 'GET',
	    url: contextPath+'/listpagedcourses.do',//分页查询,在CourseController.java中
	    cache: false,
	    height: 450,
	    striped: true,
	    pagination: true,
