...
function create(){
	...
	$("#del_flag").val("0");
	//保存方法,在HomeworkController.java中
	url = contextPath + "/savehomework.do";
	method = "POST";
...
//				console.info("current_name="+current_name+"\n"+"current_path="+new_name);
				var file_link = "<div id='cur_file_"+i+"'>" +
									...
									"&nbsp;&nbsp;" +
									"<a href='#' onclick='removeFile(\""+file_id+"\",\""+rows[0].homework_id+"\",\"cur_file_"+i+"\",\""+paths[i]+"\");'><i class='glyphicon glyphicon-trash'></i>&nbsp;删除附件</a>" +
								"</div>";
	...
	//修改方法,在HomeworkController.java中
	url = contextPath+"/updatehomework.do";
    method = "POST";
    data_form._method.value="POST";
}
...
function remove(){
	...
	if(rows.length>0){
		$.messager.confirm('Confirm', '确定要删除?', function(r) {
			if (r) {
				//删除方法,在HomeworkController.java中
				url = contextPath + "/deletehomeworks.do";
				method = "POST";
				data_form._method.value="DELETE";
				for ( var i = 0; i < rows.length; i++) {
					$("#ids").append(
							"<input type=\"hidden\" name=\"ids\" value=\"" + rows[i].homework_id
									+ "\">");
					}
				$('#data_form').form('load', rows[0]);
				$('#submit').click();
...
//	alert(file_id+"\t"+notice_id);
	if(valueIsNotEmpty(file_id)&&valueIsNotEmpty(related_id)){
		$.ajax({
			type : "POST",
			data : {attachment_id:file_id,related_id:related_id,file_path:path,table_of_attachment:"roy_homework"},
...
function loadAllCourses(){
	$.ajax({
		type: "POST",
		url: contextPath + '/listmycourses.do',
		async : false,
		success: function(result) {
			$("#homework_course").empty();
			$.each(result.rows,function(idx,item){
				$("#homework_course").append("<option value='"+item.course_name+"'>"+item.course_name+"</option>");
			});
		}
	});
}
...
//			$('#summernote').code($(this).val());
	...
	$('#data_table').bootstrapTable({
	    method: 'GET',
	    url: contextPath+'/listpagedhomeworks.do',//分页查询,在HomeworkController.java中
	    cache: false,
	    height: 450,
	    striped: true,
	    pagination: true,
...
//	    smartDisplay: false,
	    ...
	    columns: [
			...
			{
				field : 'homework_course',
				title : '所属课程',
				align : 'center'
			}, 
			{
				field : 'homework_request',
				title : '作业要求',
				align : 'center',
