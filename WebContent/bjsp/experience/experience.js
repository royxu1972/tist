...
function create(){
	...
	//保存方法,在ExperienceController.java中
	url = contextPath + "/saveexperience.do";
	method = "POST";
...
function edit(){
	...
	//修改方法,在ExperienceController.java中
	url = contextPath+"/updateexperience.do";
    method = "POST";
    data_form._method.value="POST";
}
...
function remove(){
	...
	if(rows.length>0){
		$.messager.confirm('Confirm', '确定要删除?', function(r) {
			if (r) {
				//删除方法,在ExperienceController.java中
				url = contextPath + "/deleteexperiences.do";
				method = "POST";
				data_form._method.value="DELETE";
				for ( var i = 0; i < rows.length; i++) {
					$("#ids").append(
							"<input type=\"hidden\" name=\"ids\" value=\"" + rows[i].experience_id
									+ "\">");
					}
				$('#data_form').form('load', rows[0]);
				$('#submit').click();
...
$(function(){
	...
	$('#data_table').bootstrapTable({
	    method: 'GET',
	    url: contextPath+'/listpagedexperiences.do',//分页查询,在ExperienceController.java中
	    cache: false,
	    height: 450,
	    striped: true,
	    pagination: true,
...
//			获取当前按钮的iconCls的值，根据值来判断用户点击了什么按钮
	...
	$('#submit').click(function() {
		$.ajax({
			...
			url : url,
			beforeSend:function(){
				validateForm();//在experience_validate.js中
