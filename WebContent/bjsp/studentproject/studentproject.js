...
function create(){
	...
	//保存方法,在StudentProjectController.java中
	url = contextPath + "/savestudentproject.do";
	method = "POST";
...
//				console.info("current_name="+current_name+"\n"+"current_path="+new_name);
				var file_link = "<div id='cur_file_"+i+"'>" +
									...
									"&nbsp;&nbsp;" +
									"<a href='#' onclick='removeFile(\""+file_id+"\",\""+rows[0].stu_proj_id+"\",\"cur_file_"+i+"\",\""+paths[i]+"\");'><i class='glyphicon glyphicon-trash'></i>&nbsp;删除附件</a>" +
								"</div>";
	...
	//修改方法,在StudentProjectController.java中
	url = contextPath+"/updatestudentproject.do";
    method = "POST";
    data_form._method.value="POST";
}
...
function remove(){
	...
	if(rows.length>0){
		$.messager.confirm('Confirm', '确定要删除?', function(r) {
			if (r) {
				//删除方法,在StudentProjectController.java中
				url = contextPath + "/deletestudentprojects.do";
				method = "POST";
				data_form._method.value="DELETE";
				for ( var i = 0; i < rows.length; i++) {
					$("#ids").append(
							"<input type=\"hidden\" name=\"ids\" value=\"" + rows[i].stu_proj_id
									+ "\">");
					}
				$('#data_form').form('load', rows[0]);
				$('#submit').click();
...
//	alert(file_id+"\t"+notice_id);
	if(valueIsNotEmpty(file_id)&&valueIsNotEmpty(related_id)){
		$.ajax({
			type : "POST",
			data : {attachment_id:file_id,related_id:related_id,file_path:path,table_of_attachment:"roy_student_project"},
