        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    //Linux下不要+7，windows本地使用要+7，否则tist会出现两次
                    var url = list[i].url.substring(list[i].url.indexOf("/tist/")+6,list[i].url.length);
//                    console.info(url);
                    img.setAttribute('src', urlPrefix + url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
